...
public class DbClientPostgres(Config config) : IDbClient
{
    ...
    public List<HistoryScript> GetHistoryScripts()
    {
        ...
        connection.Open();
        using var command = new NpgsqlCommand("SELECT order_number,script_name FROM script_history", connection);
    ...
    public void InsertHistoryScript(ScriptData scriptData)
    {
        using var connection = GetConnection();
        connection.Open();
        using var command = new NpgsqlCommand("INSERT INTO script_history (order_number, script_name, script_content, script_status, script_time) VALUES (@order_number, @script_name,@script_content,@script_status, @script_time)", connection);
